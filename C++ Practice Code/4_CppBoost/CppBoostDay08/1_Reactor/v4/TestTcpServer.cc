...
ThreadPool *gPool = nullptr;//全局的线程池对象
...
public:
    ...
    void process()
    {
        ...
        //
        //要让TcpConnection知道EventLoop的存在，才能将处理好之后的数据
        //发给EventLoop，所以需要再TcpConnection中天健EventLoop的指针
...
void test()
{
    ...
    TcpServer server("127.0.0.1", 8888);//创建TcpServer对象
    server.setAllCallback(std::move(onNewConnection)
                          , std::move(onMessage)
                          , std::move(onClose));//注册回调
    server.start();//启动TcpServer
}
